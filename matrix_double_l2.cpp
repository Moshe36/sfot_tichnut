    ...
    /////////////////////////////////// Matrix Operation Tools Class ///////////////////////////////////
     struct Tools {
        ...
        static void reorder_b(int k, const double *b, int ldb, double *b_tmp)
        {
         ...
         static void reorder_a(const double* a, int mm, int kk, int lda, double* a_tmp){
             for(int i = 0; i < mm; i++)
                 for(int j = 0; j < kk; j++)
         ...
         static void mult(int m, int k, int n, const double* a, const double* b, double* c, int lda, int ldb, int ldc)
         {
             ...
             int kk = std::min(L1 / 8 / 8, k);
             int mm = std::min(L2 / 8 / kk, m) / 6 * 6;
             double* b_tmp = new double[kk * 8];
             double* a_tmp = new double[kk * mm];
